...
local OPBox = Tabs.Main:AddRightGroupbox('作弊功能')
OPBox:AddLabel('警告：滥用此功能可能导致服务器崩溃或严重延迟',true)
...
local AuraBox = Tabs.Main:AddLeftGroupbox('光环')
local TPBox = Tabs.Main:AddLeftGroupbox('传送')
TPBox:AddLabel('请与卡在空中的目标交互，这样所有人都能看到并生效',true)
...
local ESPBox = Tabs.Visuals:AddRightGroupbox('透视')
local ESPSettings = Tabs.Visuals:AddLeftGroupbox('透视设置')
...
local MenuGroup = Tabs.Settings:AddLeftGroupbox('界面设置')
    local UtilityBox = Tabs.Settings:AddRightGroupbox('工具箱')
...
Tp:AddButton("传送到房间(免费)", function()
...
Tp:AddButton("传送到户外(免费)", function()
...
local MinesFloor = Tabs.Floor:AddRightGroupbox('矿井')
...
LocalPlayer.Character.Collision.Position = LocalPlayer.Character.HumanoidRootPart.Position
	...
	Text = "上帝模式", -- Text to display in the keybind menu
	NoUI = false, -- Set to true if you want to hide from the Keybind menu,
...
Default = false
	...
	Text = "自动隐藏点", -- Text to display in the keybind menu
	NoUI = false, -- Set to true if you want to hide from the Keybind menu,
...
BypassBox:AddLabel("假复活开启后无法撤销，请自行承担风险。如果你的角色变得隐形，这意味着你失去了网络所有权，假复活已损坏",true)
...
BypassBox:AddDivider()
BypassBox:AddLabel("无限十字架仅对A-60、A-120、Rush和Ambush有效，且有失败几率，请自行承担风险",true)
...
Notify("抱歉，你的执行器不支持删除Figure，因为缺少isnetworkowner", 5)
...
Notify("反作弊已绕过，这仅在过场动画或Halt之前有效",5)
...
firetouchinterest(LocalPlayer.Character.HumanoidRootPart, Item, 1)
        ...
        if Success then
            Notify("成功删除Seek，可以打开下一扇门", 3)
        else
            Notify("删除Seek失败，打开下一扇门", 3)
        end
    end
end
end
